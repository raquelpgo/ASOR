...
int main (int argc, char* argv[])
{
	...
	while (cmmd[0] != 'q') 
	{
		...
		if (select(udp_sd + 1, &set, NULL, NULL, NULL) == -1) 
		{
			printf("Error esperando los datos!\n");
			return -1;
		}

		
		if (FD_ISSET(udp_sd, &set))
			readsz = recvfrom(udp_sd, &cmmd, 2, 0, (struct sockaddr*) &input_addr, &input_len);
		else if (FD_ISSET(0, &set))
			readsz = read(0, &cmmd, 2);
